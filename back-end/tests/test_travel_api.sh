...
if echo "$LOGIN_RESPONSE" | grep -q '"success":true'; then
  ...
  if [ -z "$TOKEN" ]; then
    # 嘗試其他可能的格式
    TOKEN=$(echo $LOGIN_RESPONSE | grep -o '"token":[^,}]*' | sed 's/"token"://' | tr -d '"')
  fi

  if [ -z "$TOKEN" ]; then
    echo -e "${RED}無法從響應中提取令牌。${NC}"
    echo "請檢查 API 響應格式並更新腳本。"
    exit 1
  else
  ...
  echo -e "${RED}登入失敗，API 回應沒有成功標誌。${NC}"
  echo "回應: $LOGIN_RESPONSE"
  echo -e "${YELLOW}提示：確保後端服務正在運行且用戶登入API工作正常${NC}"
  exit 1
fi
...
echo $ALL_PLANS_RESPONSE | python -m json.tool
...
# 檢查回應是否成功並提取計劃ID
if echo "$KYOTO_RESPONSE" | grep -q '"success":true' || echo "$KYOTO_RESPONSE" | grep -q '"plan_id":\|"id":\|"_id":'; then
  KYOTO_PLAN_ID=$(echo $KYOTO_RESPONSE | grep -o '"plan_id":"[^"]*\|"id":"[^"]*\|"_id":"[^"]*' | sed 's/"[^"]*":"//')
  ...
  echo $KYOTO_DETAILS | python -m json.tool | head -n 20
...
# 檢查回應是否成功並提取計劃ID
if echo "$TOKYO_RESPONSE" | grep -q '"success":true' || echo "$TOKYO_RESPONSE" | grep -q '"plan_id":\|"id":\|"_id":'; then
  TOKYO_PLAN_ID=$(echo $TOKYO_RESPONSE | grep -o '"plan_id":"[^"]*\|"id":"[^"]*\|"_id":"[^"]*' | sed 's/"[^"]*":"//')
  ...
  echo $TOKYO_DETAILS | python -m json.tool | head -n 20
  echo -e "${YELLOW}...[輸出被截斷]...${NC}"
...
echo $REMAINING_PLANS | python -m json.tool

echo -e "\n${BLUE}===== 測試完成 =====${NC}"
echo -e "${GREEN}所有 API 功能已成功測試！${NC}"
